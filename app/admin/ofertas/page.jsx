...
export default function AdminOfertasPage() {
  ...
  const fetchOfertas = async () => {
    try {
      setLoading(true);
      const q = query(collection(db, "ofertas"), orderBy("titulo", "asc"));
      const querySnapshot = await getDocs(q);
      const data = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      setItems(data);
    } catch (error) {
      console.error("Error fetching ofertas:", error);
      Swal.fire({ icon: "error", title: "Error", text: "No se pudieron cargar las ofertas", confirmButtonColor: GOLD });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const fileName = `ofertas/${timestamp}_${file.name}`;
  ...
  const handleCreate = () => {
    setEditingItem(null);
    setFormData({ titulo: "", slug: "", descripcion: "", imagen: "", active: true, featured: false });
    setImagePreview("");
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    if (!formData.titulo.trim()) {
      Swal.fire({ icon: "warning", title: "Título requerido", text: "Por favor ingresa el título de la oferta", confirmButtonColor: GOLD });
      return;
    }
    if (!formData.imagen) {
      Swal.fire({ icon: "warning", title: "Imagen requerida", text: "Por favor sube una imagen para la oferta", confirmButtonColor: GOLD });
      return;
    }
    ...
    try {
      ...
      if (editingItem) {
        await updateDoc(doc(db, "ofertas", editingItem.id), { ...dataToSave, updatedAt: serverTimestamp() });
        Swal.fire({ icon: "success", title: "¡Actualizado!", timer: 1500, showConfirmButton: false });
      } else {
        await addDoc(collection(db, "ofertas"), { ...dataToSave, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      ...
      Swal.fire({ icon: "error", title: "Error", text: "No se pudo guardar la oferta", confirmButtonColor: GOLD });
    }
  };

  const handleToggleActive = async (item) => {
    try {
      await updateDoc(doc(db, "ofertas", item.id), { active: !item.active, updatedAt: serverTimestamp() });
      Swal.fire({ icon: "success", title: item.active ? "Desactivada" : "Activada", timer: 1500, showConfirmButton: false });
      fetchOfertas();
    } catch (error) {
  ...
  const handleToggleFeatured = async (item) => {
    try {
      await updateDoc(doc(db, "ofertas", item.id), { featured: !item.featured, updatedAt: serverTimestamp() });
      Swal.fire({ icon: "success", title: item.featured ? "Removida de destacadas" : "Añadida a destacadas", timer: 1500, showConfirmButton: false });
      fetchOfertas();
    } catch (error) {
      Swal.fire({ icon: "error", title: "Error", text: "No se pudo cambiar el estado", confirmButtonColor: GOLD });
    }
  ...
  const handleDelete = async (item) => {
    ...
    if (result.isConfirmed) {
      try {
        if (item.imagen && item.imagen.includes("firebasestorage")) {
          try { await deleteObject(ref(storage, item.imagen)); } catch (e) { console.log(e); }
        }
        await deleteDoc(doc(db, "ofertas", item.id));
        Swal.fire({ icon: "success", title: "¡Eliminada!", timer: 1500, showConfirmButton: false });
        fetchOfertas();
      } catch (error) {
        Swal.fire({ icon: "error", title: "Error", text: "No se pudo eliminar la oferta", confirmButtonColor: GOLD });
      }
    }
  };
    ...
    <div className="min-h-screen bg-[#FAF9F6] p-8">
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <h1 className={`${playfair.className} text-4xl font-medium uppercase tracking-[0.12em] text-black`}>
                Ofertas <span style={{ color: GOLD }}>Especiales</span>
              </h1>
        ...
        {loading ? (
          ...
          >
            <Tag size={48} className="mx-auto mb-6 text-gray-200" />
            <p className={`${playfair.className} text-lg font-medium uppercase tracking-widest text-black`}>
              {searchTerm ? "No se encontraron ofertas" : "Sin Ofertas"}
            </p>
            <p className="mt-2 text-xs text-gray-400">
              {searchTerm ? "Intenta con otros términos" : "Comienza agregando ofertas especiales"}
            </p>
          </motion.div>
        ) : (
          <motion.div
            ...
            {filteredItems.map((item, index) => (
              ...
              >
                ...
                <div className="absolute right-4 top-4 z-10 flex flex-col gap-2">
                  ...
                  {item.featured && (
                    <span className="px-3 py-1 text-[8px] font-black uppercase tracking-widest backdrop-blur-md shadow-sm text-white"
                ...
                <div className="relative h-52 w-full overflow-hidden bg-[#FAF9F6]">
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <h3 className={`${playfair.className} text-lg font-medium text-white leading-tight drop-shadow-lg`}>
                      {item.titulo}
                    </h3>
                  </div>
                </div>
                ...
                <div className="p-6">
                  {item.descripcion && (
                    <p className="mb-4 text-xs text-gray-500 leading-relaxed line-clamp-3">
                      {item.descripcion}
                    </p>
                  )}
            ...
            <>
              ...
              >
                ...
                <div className="mb-10 flex items-center justify-between">
                  <div>
                    ...
                    <h2 className={`${playfair.className} text-2xl font-medium uppercase tracking-[0.12em] text-black`}>
                      Oferta <span style={{ color: GOLD }}>Especial</span>
                    </h2>
                  </div>
                  <button
                    onClick={() => setShowModal(false)}
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setFormData(prev => ({ ...prev, titulo, slug: generateSlug(titulo) }));
                      }}
                  ...
                  <div className="space-y-3">
                    ...
                    {imagePreview ? (
                      <div className="relative group">
                        <div className="relative h-56 overflow-hidden border border-gray-100 bg-white">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="flex items-center justify-between border border-gray-100 bg-[#FAF9F6] p-6">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-black">Oferta Activa</span>
                      </div>
                ...
                <div className="mt-10 flex gap-4">
                  ...
                  >
                    <Save size={16} />
                    {editingItem ? "Guardar Cambios" : "Crear Oferta"}
                  </button>
                </div>
